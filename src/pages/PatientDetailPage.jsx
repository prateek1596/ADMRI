...
import { RiskGauge, TrendChart, ModalityBar, TrainingChart, DomainRadar, ForecastChart, ConfidenceBar } from "../components/charts/Charts";
...
export function PatientDetailPage({
  ...
  return (
    <motion.div key="detail" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 5 }}>
            ...
            {patient.riskHistory?.length > 0 && (
              <RiskBadge score={patient.riskHistory[patient.riskHistory.length - 1]} />
            )}
          </div>
      ...
      {/* ML status banner */}
      {mlState.loading && (
        <div style={{ padding: "10px 16px", background: `${T.accentAlt}10`, border: `1px solid ${T.accentAlt}33`, borderRadius: 12, marginBottom: 14, display: "flex", alignItems: "center", gap: 12, fontSize: 13 }}>
          <Spinner size={16} color={T.accentAlt} />
          <span style={{ color: T.accentAlt }}>Checking browser cache for saved models...</span>
        </div>
      )}
      {mlState.training && (
        <div style={{ padding: "14px 18px", background: `${T.accent}08`, border: `1px solid ${T.accent}33`, borderRadius: 12, marginBottom: 14 }}>
            ...
            return (
              <div key={key} style={{ marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 3 }}>
                  <span style={{ color: isDone ? T.safe : isCurrent ? T.accent : T.muted, fontWeight: isCurrent ? 700 : 400 }}>
                ...
                <div style={{ height: 3, borderRadius: 3, background: T.border, overflow: "hidden" }}>
                  <div style={{ width: `${pct}%`, height: "100%", borderRadius: 3, background: isDone ? T.safe : isCurrent ? T.accent : T.border, transition: "width 0.3s" }} />
                </div>
              </div>
            );
          })}
      ...
      {mlState.trained && !mlState.training && !mlState.loading && (
        <div style={{ padding: "8px 16px", background: `${T.safe}10`, border: `1px solid ${T.safe}33`, borderRadius: 12, marginBottom: 14, fontSize: 12, color: T.safe }}>
          {mlState.fromCache
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            style={{ padding: "6px 12px", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 11, background: `${T.safe}18`, color: T.safe }}
            onClick={() => exportPatientReport(patient, notes, getAssessmentHistory(patient.id), doctorName || "Clinician")}
          >
            📄 Export PDF
          </button>
          <button
            title="Delete patient"
            style={{ padding: "6px 12px", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 11, background: `${T.danger}18`, color: T.danger }}
            onClick={() => { if (window.confirm(`Permanently delete ${patient.name}? This cannot be undone.`)) onRemovePatient?.(patient.id); }}
          >
            🗑 Delete
          </button>
        </div>
      ...
      <AnimatePresence mode="wait">
        {tab === "overview"  && <OverviewTab   key="ov"  patient={patient} patNotes={patNotes} mlState={mlState} onGoAssess={() => setTab("assess")} />}
        ...
        {tab === "history"   && <HistoryTab    key="hi"  patient={patient} patNotes={patNotes} assessmentHistory={getAssessmentHistory?.(patient.id) || []} />}
      </AnimatePresence>
    </motion.div>
  );
}
...
function OverviewTab({ patient, patNotes, mlState, onGoAssess }) {
  ...
  async function sendChat() {
    ...
    const updated = [...aiChat, { role: "user", content: msg }];
    setAiChat(updated);
    setChatLoading(true);

    const sys = `You are a compassionate clinical AI in the ADMRI system. Patient: ${patient.name}, age ${patient.age}, diagnosis: ${patient.diagnosis}. Latest ADMRI score: ${lastScore ?? "not yet assessed"}/100. Give warm, evidence-based responses. Never diagnose. Keep replies to 2-3 sentences. Crisis line if needed: iCall 9152987821.`;

    if (!ANTHROPIC_API_KEY) {
      setAiChat(prev => [...prev, { role: "assistant", content: "⚙️ AI chat needs an API key. Open src/config.js and paste your Anthropic API key to enable this feature. Get one free at console.anthropic.com" }]);
    ...
    try {
      ...
      if (data.error) {
        setAiChat(prev => [...prev, { role: "assistant", content: `API error: ${data.error.message}` }]);
      } else {
        setAiChat(prev => [...prev, {
          role: "assistant",
          content: data.content?.map(b => b.text || "").join("") || "I'm here to help.",
        }]);
      }
    } catch (e) {
      setAiChat(prev => [...prev, { role: "assistant", content: "Network error — check your connection and API key in src/config.js." }]);
    }
    setChatLoading(false);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        {/* Risk gauge */}
        <div style={card}>
          <div style={{ fontSize: 10, color: T.muted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 14 }}>
            Current ADMRI Score
          </div>
          {lastScore !== undefined ? (
            <>
            ...
            <div style={{ textAlign: "center", padding: "30px 0", color: T.muted, fontSize: 13 }}>
              No assessments yet.{" "}
              <span style={{ color: T.accent, cursor: "pointer" }} onClick={onGoAssess}>
                Run first →
              </span>
            </div>
          )}
        ...
        <div style={card}>
          ...
          {patNotes.length === 0 && <div style={{ color: T.muted, fontSize: 13 }}>No notes yet.</div>}
      ...
      {/* AI Chat */}
      <div style={card}>
        <div style={{ fontSize: 10, color: T.muted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 14, display: "flex", gap: 8, alignItems: "center" }}>
          <span style={{ width: 7, height: 7, borderRadius: "50%", background: T.safe, display: "inline-block", boxShadow: `0 0 6px ${T.safe}` }} />
          AI Clinical Support
          <span style={{ fontWeight: 400, textTransform: "none", color: T.muted, fontSize: 11 }}>
            — Context-aware assistant for {patient.name.split(" ")[0]}
          </span>
        </div>
        <div style={{ height: 260, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, marginBottom: 12 }}>
          {aiChat.length === 0 && (
            <div style={{ color: T.muted, fontSize: 13, textAlign: "center", paddingTop: 60 }}>
              Ask about treatment strategies for {patient.name.split(" ")[0]}…
            </div>
          )}
          {aiChat.map((m, i) => (
            <div key={i} style={{
              ...
              background: m.role === "user" ? `${T.doctorAccent}22` : T.surface,
              border: `1px solid ${m.role === "user" ? T.doctorAccent + "44" : T.border}`,
          ...
          {chatLoading && (
            <div style={{ alignSelf: "flex-start", padding: "9px 14px", borderRadius: "16px 16px 16px 4px", background: T.surface, border: `1px solid ${T.border}`, fontSize: 12, color: T.muted }}>
              Thinking…
            </div>
          )}
          <div ref={chatEnd} />
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input style={inp} placeholder="Ask about CBT techniques, medication, treatment options…"
            value={chatInput} onChange={e => setChatInput(e.target.value)}
            onKeyDown={e => e.key === "Enter" && sendChat()} />
          <button style={{ ...btn("primary", "sm"), whiteSpace: "nowrap" }} onClick={sendChat} disabled={chatLoading}>
            Send
          </button>
        </div>
      </div>
...
function NotesTab({ patNotes, onAddNote, onRemoveNote }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      {patNotes.length === 0 && (
        <div style={{ ...card, textAlign: "center", color: T.muted, padding: 48 }}>No notes yet.</div>
      )}
        ...
        <motion.div key={note.id}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <span style={chip(NOTE_TYPE_COLORS[note.type] || T.accent)}>{note.type}</span>
              <span style={{ fontSize: 12, color: MOOD_COLORS[note.mood] || T.muted }}>● {note.mood}</span>
            </div>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <span style={{ fontSize: 12, color: T.muted }}>{note.date}</span>
              {onRemoveNote && (
                <button style={{ background: "none", border: "none", cursor: "pointer", color: T.muted, fontSize: 13, padding: "2px 4px" }}
                  title="Delete note"
                  onClick={() => { if (window.confirm("Delete this note?")) onRemoveNote(note.id); }}>
                  🗑
                </button>
              )}
            </div>
...
function AssessTab({ patient, mlState, onRunAssessment }) {
  ...
  function saveDraft() {
    try { localStorage.setItem(DRAFT_KEY, JSON.stringify({ step, answers, journal, behavioral })); setHasDraft(true); } catch {}
  }
  function clearDraft() {
  ...
  function runML() {
    ...
    const fullSnapshot = {
      score, adaptive, questScore: qs, sentimentScore: sentScore, behaviouralScore: bs,
  ...
  async function sendChat() {
    ...
    setAiChat(updated);
    setChatLoading(true);
    const sys = `You are a clinical AI assistant in ADMRI. Patient: ${patient.name}, ADMRI score: ${results ? results.score : "N/A"}. Give evidence-based, brief (2-3 sentence) clinical responses. Crisis line: iCall 9152987821.`;

    if (!ANTHROPIC_API_KEY) {
      setAiChat(prev => [...prev, { role: "assistant", content: "⚙️ AI chat needs an API key. Open src/config.js and paste your Anthropic API key." }]);
      setChatLoading(false);
      return;
    }
    ...
    try {
      const res  = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, system: sys, messages: updated }),
      });
      const data = await res.json();
      if (data.error) {
        setAiChat(prev => [...prev, { role: "assistant", content: `API error: ${data.error.message}` }]);
      } else {
        setAiChat(prev => [...prev, { role: "assistant", content: data.content?.map(b => b.text || "").join("") || "I'm here." }]);
      }
    } catch {
      setAiChat(prev => [...prev, { role: "assistant", content: "Network error — check your connection and API key in src/config.js." }]);
    }
  ...
  if (!results) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      {/* Draft banner */}
      {hasDraft && (
        <div style={{ padding: "10px 16px", background: `${T.accentAlt}12`, border: `1px solid ${T.accentAlt}33`, borderRadius: 12, marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12 }}>
          <span style={{ color: T.accentAlt, fontWeight: 600 }}>💾 Draft restored — your previous progress is loaded</span>
          <button style={{ background: "none", border: "none", cursor: "pointer", color: T.muted, fontSize: 11 }} onClick={() => { setStep(0); setAnswers({}); setJournal(""); setBehavioral({ sleepHours: 7, screenTime: 3, exerciseMinutes: 30, socialInteractions: 3, appetiteChange: false }); clearDraft(); }}>
            ✕ Discard draft
          </button>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 22 }}>
        ...
        <button style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", color: T.muted, fontSize: 11, fontFamily: "'DM Sans', sans-serif" }} onClick={saveDraft} title="Save progress as draft">
          💾 Save draft
        </button>
      </div>
      ...
      <AnimatePresence mode="wait">
        {step === 0 && (
          <motion.div key="q" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div style={card}>
              <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 17, marginBottom: 4 }}>
                  ...
                  <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                    {OPTIONS.map(o => (
                      <button key={o.value}
                        ...
                        style={{
                          padding: "7px 12px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600, transition: "all 0.12s", fontFamily: "'DM Sans', sans-serif",
              ...
              <button style={btn("primary")} onClick={() => setStep(1)}
          ...
          <motion.div key="j" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div style={card}>
              ...
              {journal.length > 10 && (() => {
                ...
                return (
                  <div style={{ marginTop: 10, padding: "10px 14px", background: T.surface, borderRadius: 8, fontSize: 12 }}>
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: det.crisisFlags.length > 0 ? 8 : 0 }}>
                      ...
                      <span style={{ color: T.muted }}>Emotion: <strong style={{ color: det.dominantEmotion === "distress" ? T.danger : det.dominantEmotion === "positive" ? T.safe : T.warn }}>{det.dominantEmotion}</strong></span>
                      <span style={{ color: T.muted }}>Variance: <strong style={{ color: T.textSoft }}>{det.variance}</strong></span>
                    </div>
                    {det.crisisFlags.length > 0 && (
                      <div style={{ padding: "7px 10px", background: `${T.danger}15`, border: `1px solid ${T.danger}44`, borderRadius: 8, color: T.danger, fontWeight: 600 }}>
        ...
        {step === 2 && (
          <motion.div key="b" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div style={card}>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", marginBottom: 18, fontSize: 13 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button style={btn("primary")} onClick={() => { clearDraft(); runML(); }}>
                  {mlState.trained ? "Generate ADMRI Score (Neural Net) →" : "Generate ADMRI Score (Fallback) →"}
                </button>
              </div>
            </div>
          </motion.div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div style={{ display: "flex", gap: 6, marginBottom: 18, flexWrap: "wrap" }}>
        {[["overview","Score Overview"],["recs","CBT Interventions"],["model","Model Info"],["chat","AI Discussion"]].map(([id, label]) => (
          <button key={id} style={tabBtn(dashTab === id)} onClick={() => setDashTab(id)}>{label}</button>
        ))}
        <button style={{ ...btn("", "sm"), marginLeft: "auto" }} onClick={() => { setResults(null); setStep(0); setAnswers({}); }}>
          Re-assess
        </button>
      </div>
        ...
        <div>
          ...
          {results.anomaly && (
            <div style={{
              ...
              color: results.anomaly.severity === "critical" ? T.danger : T.warn, fontWeight: 600,
            }}>
              🔔 Anomaly detected ({results.anomaly.change} pts, z={results.anomaly.zScore}σ): {results.anomaly.message}
            </div>
          )}

          {/* Crisis language alert */}
          {results.sentDetails?.crisisFlags?.length > 0 && (
            <div style={{ padding: "12px 16px", marginBottom: 14, background: `${T.danger}12`, border: `1px solid ${T.danger}44`, borderRadius: 12, fontSize: 13, color: T.danger, fontWeight: 600 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 14 }}>
            <div style={card}>
              <div style={{ fontSize: 10, color: T.muted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>
              ...
              {results.adaptive && (
                <div style={{ textAlign: "center", fontSize: 12, color: T.muted, marginTop: 8 }}>
                  7-session adaptive avg: <strong style={{ color: T.accentAlt }}>{results.adaptive}</strong>
                </div>
              )}
              <TrendChart history={results.history} />
              <ForecastChart history={results.history} forecast={results.forecast} />
              {results.risk.tier >= 4 && (
                <div style={{ marginTop: 12, padding: 12, background: `${T.danger}10`, border: `1px solid ${T.danger}33`, borderRadius: 10, fontSize: 12, color: T.danger }}>
                  ⚠️ {results.risk.description}<br />Crisis line: <strong>iCall 9152987821</strong>
                </div>
              )}
            </div>
            ...
            <div>
              <div style={card}>
                ...
                <ModalityBar label="NLP Sentiment Analysis"         score={results.ss}  color={results.sentDetails?.dominantEmotion === "distress" ? T.danger : T.accentAlt} icon="💭" />
      ...
      {dashTab === "recs" && (
        <div>
          <div style={{ ...card, background: `${T.accentAlt}0d`, border: `1px solid ${T.accentAlt}33`, marginBottom: 14 }}>
            <div style={{ fontSize: 13, color: T.accentAlt, fontWeight: 700 }}>CBT Intervention Engine — Evidence-Graded</div>
            <div style={{ fontSize: 12, color: T.muted, marginTop: 4 }}>Personalized to {patient.name.split(" ")[0]}'s dominant symptom domain.</div>
          </div>
          {results.recs.map((rec, i) => {
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                ...
                <div style={{ fontSize: 13, color: T.muted, lineHeight: 1.65, marginBottom: 8 }}>{rec.description}</div>
      ...
      {dashTab === "model" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            {[
              { key: "depnet",    label: "Model 1 — DepNet",    color: T.danger,    desc: "Specialised on PHQ-9 depression features (anhedonia, mood, self-worth, fatigue, concentration). Architecture: Dense(64)→BN→Dense(32)→Dropout(0.3)→Dense(16)→Sigmoid" },
              { key: "anxnet",    label: "Model 2 — AnxNet",    color: T.warn,      desc: "Specialised on GAD-7 anxiety + SCARED features (nervousness, worry, fear, sleep, sentiment). Architecture: Dense(64)→BN→Dropout(0.25)→Dense(32)→Dense(16)→Sigmoid" },
              { key: "sleepnet",  label: "Model 3 — SleepNet",  color: T.accentAlt, desc: "Specialised on ISI insomnia + behavioural biomarkers (sleep hours, screen time, exercise, social interactions, appetite). Architecture: Dense(48)→BN→Dense(24)→Dropout(0.25)→Dense(12)→Sigmoid" },
              { key: "fusionnet", label: "Model 4 — FusionNet", color: T.accent,    desc: "Stacked meta-learner. Takes all 20 features + outputs of models 1-3 (23 inputs total). Learns optimal weighting. Architecture: Dense(128)→BN→Dense(64)→Dropout(0.35)→Dense(32)→Dropout(0.2)→Sigmoid" },
            ].map(m => (
              <div key={m.key} style={{ ...card, borderLeft: `3px solid ${m.color}`, marginBottom: 0 }}>
                ...
                <div style={{ fontSize: 12, color: T.muted, lineHeight: 1.65, marginBottom: 10 }}>{m.desc}</div>
                {mlState.trainLogs[m.key]?.length > 0 && (
                  <div style={{ fontSize: 11, color: T.muted }}>
                    Final loss: <span style={{ color: m.color, fontWeight: 700 }}>{mlState.trainLogs[m.key].slice(-1)[0].loss}</span>
                    &nbsp;·&nbsp; MAE: <span style={{ color: T.safe }}>{mlState.trainLogs[m.key].slice(-1)[0].mae}</span>
                  </div>
                )}
              </div>
            ))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div style={card}>
              ...
              {[
                ...
                ["Severity split",  "None 50%, Mild 22%, Moderate 15%, High 8%, Severe 5%" ],
                ["References",     "Kroenke 2001, Spitzer 2006, Morin 2011, Birmaher 1997"],
                ["Epochs",         "60 per model with 15% validation split"   ],
            ...
            <div style={card}>
              <div style={{ fontSize: 10, color: T.muted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>
                Per-Model Scores (last assessment)
              </div>
              {results?.confidence?.modelScores && Object.entries({
                ...
                return (
                  <div key={label} style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ height: 5, borderRadius: 3, background: T.border, overflow: "hidden" }}>
                      <div style={{ width: `${score}%`, height: "100%", background: colors[i], borderRadius: 3, transition: "width 1s" }} />
                    </div>
                  </div>
                );
              })}
              ...
              {mlState.fromCache && (
                <div style={{ marginTop: 14, padding: 10, background: `${T.accent}0d`, borderRadius: 8, fontSize: 11, color: T.muted }}>
      ...
      {dashTab === "chat" && (
        <div style={card}>
          <div style={{ fontSize: 10, color: T.muted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 14 }}>
            AI Clinical Discussion — Score {results.score} · {results.risk.label}
          </div>
          <div style={{ height: 280, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, marginBottom: 12 }}>
            {aiChat.length === 0 && <div style={{ color: T.muted, fontSize: 13, textAlign: "center", paddingTop: 50 }}>Discuss treatment for {patient.name.split(" ")[0]} based on today's score…</div>}
            {aiChat.map((m, i) => (
              <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "80%", padding: "9px 14px", borderRadius: m.role === "user" ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: m.role === "user" ? `${T.doctorAccent}22` : T.surface, border: `1px solid ${m.role === "user" ? T.doctorAccent + "44" : T.border}`, fontSize: 13, lineHeight: 1.6, color: T.text }}>
                {m.content}
              </div>
            ))}
            {chatLoading && <div style={{ alignSelf: "flex-start", padding: "9px 14px", background: T.surface, borderRadius: "16px 16px 16px 4px", fontSize: 12, color: T.muted }}>Thinking…</div>}
            <div ref={chatEnd} />
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <input style={inp} placeholder="Ask about next steps, medication, family involvement…" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && sendChat()} />
            <button style={{ ...btn("primary", "sm"), whiteSpace: "nowrap" }} onClick={sendChat} disabled={chatLoading}>Send</button>
          </div>
        </div>
      )}
    </motion.div>
            ...
            <>
              ...
              <div style={{ marginTop: 16 }}>
                {patient.riskHistory.map((score, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${T.border}`, fontSize: 13 }}>
                      <div>
                        <span style={{ color: T.muted }}>Session {i + 1}</span>
                        {snap?.date && <span style={{ color: T.muted, fontSize: 11, marginLeft: 8 }}>{snap.date}</span>}
                      </div>
                      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                        <span style={{ fontWeight: 600 }}>{score}/100</span>
                        <RiskBadge score={score} />
                        {snap && (
                          <button style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, cursor: "pointer", color: T.accent, fontSize: 11, padding: "2px 8px" }}
        ...
        <div>
          ...
          {selected && (
            <div style={{ ...card, border: `1px solid ${T.accent}33`, marginBottom: 14 }}>
              <div style={{ fontSize: 10, color: T.accent, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                {[
                  ...
                  ["CI Range",     selected.confidence ? `${selected.confidence.lower}–${selected.confidence.upper}` : "—"],
                ].map(([label, val]) => (
                  <div key={label} style={{ padding: "8px 10px", background: T.surface, borderRadius: 8 }}>
                    <div style={{ fontSize: 10, color: T.muted, fontWeight: 700, marginBottom: 3 }}>{label}</div>
              ...
              {selected.journal && (
                <div>
                  <div style={{ fontSize: 10, color: T.muted, fontWeight: 700, marginBottom: 6 }}>JOURNAL ENTRY</div>
                  <div style={{ fontSize: 12, color: T.muted, lineHeight: 1.65, padding: "10px 12px", background: T.surface, borderRadius: 8 }}>
                    {selected.journal}
                  </div>
                </div>
              )}
            ...
            {patNotes.map((note, i) => (
              <div key={note.id} style={{ display: "flex", gap: 12, marginBottom: 14 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ width: 9, height: 9, borderRadius: "50%", background: T.accent, marginTop: 3, flexShrink: 0 }} />
                  {i < patNotes.length - 1 && <div style={{ width: 1, flex: 1, background: T.border, margin: "4px 0" }} />}
                </div>
